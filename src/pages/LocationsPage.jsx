...
const LocationsPage = () => {
  ...
  return (
    <div>
      ...
      {/* Tabla de Ubicaciones (Vista Desktop) */}
      {!loading && !error && (
        <>
          ...
          <div className="md:hidden space-y-4">
            {locations.length === 0 ? (
              <div className="bg-white p-12 text-center rounded-lg shadow-sm border border-gray-100">
                <MapPin size={48} className="mx-auto mb-3 text-gray-300" />
                <p className="text-gray-500 text-lg">No hay ubicaciones</p>
              </div>
            ) : (
              locations.map((location) => (
                <div key={location.id} className="bg-white p-4 rounded-xl shadow-sm border border-gray-100">
                  <div className="flex items-center gap-4 mb-4">
                    <div className="flex-shrink-0 h-14 w-14 bg-turi-green-100 rounded-full flex items-center justify-center">
                      <MapPin size={28} className="text-turi-green-dark" />
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex justify-between items-start">
                        <h4 className="text-lg font-bold text-gray-900 truncate">#{location.id} - {location.name}</h4>
                        <span className={`px-2 py-0.5 text-[10px] font-bold uppercase rounded-full ${
                          location.is_active ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'
                        }`}>
                          {location.is_active ? 'Activo' : 'Baja'}
                        </span>
                      </div>
                    </div>
                  </div>
